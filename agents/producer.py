...
class ProducerAgent(Agent):
    ...
    def think(self, state: MarketState) -> list[str]:
        ...
        if self.inventory > 0:
            thoughts.append(
                f"Offering entire stock to market: ASK {self.inventory} @ ${ask_price:.2f} "
                f"({self.sell_discount:.0%} of market to ensure fills)."
            )
            self._pending_orders = [
                Order(self.agent_id, OrderSide.ASK, ask_price, self.inventory, state.tick)
            ]
        else:
            thoughts.append("No stock to sell this tick. Holding.")
            self._pending_orders = []
